...
module.exports.endTurn = async function (pmId) {
    try {
        ...
        if (opponent.pm_state_id == 4) {
            let sqlDeath =  `delete from deck 
            where (deck_pm_id = $1 or deck_pm_id = $2)  
            and (deck_card_hp <= 0 and deck_pos_id != 1)`;
            ...
            await pool.query(sqlDeath, [pmId, opponent.pm_id]);
...
module.exports.playCardFromHand = async function (pmId, deckId)
{
	try
	{
        ...
        let sql2 = `select * from deck where deck_id = $1`;
        let result2 = await pool.query(sql2,[deckId]);
		if (result.rowCount > 0 && result2.rows[0].deck_card_hp == 0)
		{   
            
            Spells(pmId, deckId, result2.rows[0].deck_card_id);
...
module.exports.createRandomCards = async function (pmId,nCards) {
    try {
        for(let i=0; i < nCards; i++) {
            let sql = `Select * from card where crd_id < 10
...
async function Spells(pmId, deckId, cardId){
    try{
        
        // get player match info 
        sqlmatchId = `select pm_match_id from playermatch where pm_id = $1`
        let matchId = await pool.query(sqlmatchId,[pmId])
        let truematchId = matchId.rows[0].pm_match_id


        // get opponent info
        let sqlopponent = `select * from playermatch 
                          where pm_match_id = $1
                          and pm_player_id != $2`
        let opponent = await pool.query(sqlopponent,[truematchId, pmId]);
        let opPmId = opponent.rows[0].pm_id;

        switch(cardId){
            case(6):
                    let sqlhealthpotion = `Update deck set deck_card_hp = deck_card_hp + 2 where deck_pm_id = $1 and deck_pos_id > 1 and deck_card_id != $2`
                    await pool.query(sqlhealthpotion,[pmId, cardId]);
            break;
            ...
            case(8):
                    let sqlenrage = `Update deck set deck_card_id = 10 where deck_pm_id = $1 and deck_pos_id > 1 and deck_card_id != $2`
                    await pool.query(sqlenrage,[pmId, cardId]);
            break;
